...
BuildArch="amd64"
BuildOutputDir="dist/"
BuildTarget="./cmd/entrypoints/main.go"

function print_help {
  echo "-- | Build notetkr Go module | --"
  echo ""
  echo "[ Flags ]"
  echo "  --help: Print this help menu"
  echo ""
  echo "  --bin-name (default: nt): Name for the binary output"
  echo "  --build-os (default: linux): Target OS to build for"
  echo "  --build-arch (default: amd64): Target CPU architecture to build for"
  echo "  --build-output-dir (default: dist/): Path to binary output directory"
  echo "  --build-target (default: ./cmd/entrypoints/main.go): Path to module entrypoint"
}
